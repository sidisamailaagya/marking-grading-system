...
<head>
  ...
  <style>
    ...
    @media (max-width:1100px){ .grid{ grid-template-columns:1fr } .form-grid{ grid-template-columns:1fr } }
  </style>
</head>
<body>
  <!-- Sidebar -->
  ...
  <div class="pcoded-main-container">
    <div class="pcoded-content">
      ...
      <div class="main container">
        <div class="grid">
          <div class="card">
            <div style="font-weight:800;color:#2d3748;margin-bottom:.5rem;">Profile Information</div>
            <div class="form-grid">
              <input id="name" class="input" placeholder="Full Name" value="Dr. Sarah Johnson" readonly />
              <input id="faculty" class="input" placeholder="Faculty" value="Science" readonly />
              <input id="department" class="input" placeholder="Department" value="Computer Science" readonly />
              <input id="email" class="input" placeholder="Email" value="sarah.johnson@university.edu" readonly />
            </div>
          </div>
          <div class="card">
            <div style="font-weight:800;color:#2d3748;margin-bottom:.5rem;">Change Password</div>
            <div class="form-grid">
              <input id="currentPass" class="input" type="password" placeholder="Current Password" />
  ...
  <script>
    const initProfile = {
      name:'Dr. Sarah Johnson', email:'sarah.johnson@university.edu', faculty:'Science', department:'Computer Science'
    };

    function saveProfile(){}
    function resetProfile(){ Object.entries(initProfile).forEach(([k,v]) => { const el=document.getElementById(k); if (el) el.value=v; }); }
    function changePassword(){
      const n = document.getElementById('newPass').value.trim();
      const c = document.getElementById('confirmPass').value.trim();
      if (!n || n.length<8){ alert('New password must be at least 8 characters.'); return; }
    ...
    window.addEventListener('load', resetProfile);
  </script>
</body>
</html>